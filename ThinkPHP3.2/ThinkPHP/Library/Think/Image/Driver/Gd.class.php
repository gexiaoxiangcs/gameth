...
class Gd{
    ...
    public function thumb($width, $height, $type = Image::IMAGE_THUMB_SCALE){
        ...
        switch ($type) {
            ...
            /* 最佳缩放裁剪,缩放原始图片最小边包裹缩略图后居中裁剪 */
            case Image::IMAGE_THUMB_OPTIMUM:
                //原图尺寸小于缩略图尺寸则不进行缩略
                //if($w < $width && $h < $height) return;

                //计算缩放比例
                $scale = $w/$width > $h/$height ? $height/$h : $width/$w;

                //设置缩略图的坐标及宽度和高度
                $neww = $w * $scale;
                $newh = $h * $scale;
                $x = ($neww-$width)/2/$scale;
                $y = 0;//($newh-$height)/2/$scale;//20161201改为从头部裁剪
                ...
                do{
                    ...
                    //裁剪
                    imagecopyresampled($img, $this->img, 0, 0, $x, $y, $neww, $newh, $w, $h);
                    imagedestroy($this->img); //销毁原图
                    $this->img = $img;
                } while(!empty($this->gif) && $this->gifNext());
